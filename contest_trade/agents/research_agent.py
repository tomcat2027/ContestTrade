...
class ResearchAgent:
    ...
    async def _plan(self, state: ResearchAgentState) -> ResearchAgentState:
        ...
        logger.info(f"[耗时] plan 完成: {step_elapsed:.2f}秒")
        return state
    ...
    async def _call_tool(self, state: ResearchAgentState) -> ResearchAgentState:
        ...
        selected_tool = state["selected_tool"]
        try:
            logger.info(f"[耗时] 开始调用工具: {selected_tool.get('tool_name', 'unknown')}")
            ...
            logger.info(f"[耗时] 工具 {tool_name} 调用完成")
        except Exception as e:
        ...
        step_elapsed = time.time() - step_start
        logger.info(f"[耗时] call_tool 完成: {step_elapsed:.2f}秒, 累计调用次数: {state['tool_call_count']}")
        return state
