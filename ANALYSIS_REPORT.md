...
- 补齐 setuptools 构建配置，`contesttrade` 命令和标准 `contest_trade.*` 包导入可直接使用，不再依赖修改 `sys.path`。
- 静默任务增加非阻塞进程锁，重复调度返回退出码 75，不会并发写缓存和报告。
- 增加全局运行超时、按日轮转日志和原子 `last_run.json` 健康记录。
- 将运行结果区分为 `success`、`degraded` 和 `failed`，记录 Agent 失败数、输出信号数与报告路径。
- 新增 `contesttrade doctor`，支持近期运行状态和配置的严格健康检查。
- Web 新增 `/api/health`，成功或降级返回 HTTP 200，失败或状态无效返回 HTTP 503。
- 提供 macOS launchd 模板、Linux cron 启动脚本与无人值守运维文档。
- 新增 GitHub Actions，在 Python 3.10/3.13 上验证锁文件安装、包入口、编译和单元测试。
...
| 全项目 `compileall` | 通过 |
| `git diff --check` | 通过 |
| CLI `config` / `version` | 通过，版本 1.2.0 |
...
| 真实受保护静默运行 | 通过，102.86 秒，三份报告落盘，状态 `success` |
| 删除旧模块后的公司初始化 | 通过，4 个数据 Agent + 2 个研究 Agent |
| 离线报告回放 | 通过，Markdown/JSON 文件名与内容可解析 |
| `uv lock --check` / `uv pip check` | 通过，锁文件一致且依赖兼容 |
| 标准包导入 / `contesttrade` 入口 | 通过，不再修改 `sys.path` |
| 运行锁、健康记录、结果分级 | 通过单元测试 |
| launchd plist | `plutil -lint` 通过 |
| wheel/sdist 构建与包数据检查 | 通过 |
| Web `/api/reports` 最新报告回放 | HTTP 200，发现 2 个日期分组 |
