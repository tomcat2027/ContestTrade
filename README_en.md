...
---
# ContestTrade: A-Share Multi-Agent Research and Signal Aggregation
...
ContestTrade uses a structured multi-stage pipeline to simulate an investment firm's decision process: Data Agents extract factors, Research Agents propose candidate trades, and deterministic rules aggregate, validate, and rank the resulting signals.

1.  **Data Processing Stage:** First, raw market data from multiple sources is fed into the **Data Team**. Multiple Data Analysis Agents work in parallel to refine this raw data into structured "textual factors" that are passed together to the Research Team.

2. **Research and Decision-Making Stage:** Data factors are passed to the **Research Team**. Multiple Research Agents, each guided by distinct trading beliefs and financial toolsets, analyze the factors in parallel and submit trading proposals.
3. **Deterministic Signal Aggregation:** The system validates symbols, actions, probabilities, evidence, and timestamps; merges duplicate proposals by symbol; resolves action conflicts; and ranks candidates using probability, action consensus, evidence quality, and multi-agent coverage. Historical performance-based agent weighting remains a later-stage enhancement.
...
| :----------------------- | :----------------------------------------- | :------- |
| `BOCHA_API_KEY`          | Bocha search engine key                    | No       |
| `SERP_API_KEY`           | Serper search engine key                   | No       |
