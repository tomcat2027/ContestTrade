...
定时器在周一至周五 08:00（`Asia/Shanghai`）首次唤醒，之后每小时执行一次恢复检查直至 23:00；服务器启动两分钟后也会检查一次。运行脚本会查询沪深交易日历，节假日正常跳过；当天已有成功或降级报告时也会立即退出，不会重复调用模型。若上次运行被重启中断，补跑会复用原触发时间，从已保存的 Agent 结果继续计算。
